...
let file =
  ...
  let set_file s =
    if not (Filename.check_suffix s ".mml") then raise (Arg.Bad "no .mml extension");
    file := Some s
  in
...
let () =
  ...
  try
    ...
    let v, mem = Interpreter.eval_prog prog in
    Mmlpp.print_end_value std_formatter (v, t, mem);
    exit 0
  with
